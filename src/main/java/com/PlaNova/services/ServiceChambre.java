...
public class ServiceChambre implements IService<Chambre> {
    ...
    public void ajouter(Chambre chambre) throws SQLDataException {
        String sql = "INSERT INTO chambre (type_chambre, capacite, prix_chambre, statut_chambre, id_hotel) "
                + "VALUES (?, ?, ?, ?, ?)";
    ...
    public void modifier(Chambre chambre) throws SQLDataException {
        String sql = "UPDATE chambre SET type_chambre=?, capacite=?, prix_chambre=?, statut_chambre=?, id_hotel=? "
                + "WHERE id_chambre=?";
        ...
        try {
            ...
            ps.setString(4, chambre.getStatutChambre());
            ps.setInt(5, chambre.getIdHotel());
            ps.setInt(6, chambre.getIdChambre());
            ps.executeUpdate();
        } catch (SQLException e) {
