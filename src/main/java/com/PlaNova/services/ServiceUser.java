...
import com.PlaNova.utils.PasswordUtils;
...
public class ServiceUser implements IService<User> {
    ...
    public void ajouter(User user) throws SQLException {
        String sqlUser = "INSERT INTO utilisateur (nom, prenom, email, mot_de_passe, pays, imageurl, type_utilisateur) VALUES (?, ?, ?, ?, ?, ?, ?)";
        ...
        try {
            ...
            pstmt.setString(7, resolveUserType(user));
    ...
    private String resolveUserType(User user) {
        ...
        if (user instanceof Moderateur)
            return "employee";
        return user.getTypeUtilisateur();
    }
    ...
    public void modifier(User user) throws SQLException {
        ...
        final String sqlWithPassword = "UPDATE utilisateur SET nom = ?, prenom = ?, email = ?, mot_de_passe = ?, pays = ?, imageurl = ?, type_utilisateur = ? WHERE id_utilisateur = ?";
        final String sqlWithoutPassword = "UPDATE utilisateur SET nom = ?, prenom = ?, email = ?, pays = ?, imageurl = ?, type_utilisateur = ? WHERE id_utilisateur = ?";
        ...
        try {
            ...
            if (updatePassword && passwordToSave != null) {
                ...
                pstmt.setString(6, resolveUserType(user));
    ...
    // Méthode supplémentaire : Vérifier si une matricule existe dans la base de
    // données
    ...
    /**
     * Find user by email only (no password check). TRIM on column so spaces in DB
     * don't block match; case-insensitive.
     */
    ...
    public User authenticate(String email, String motDePasse) throws SQLException {
        ...
        if (user.isBlocked()) {
            throw new SQLException(
                    "COMPTE_BLOQUE:Votre compte a été bloqué par l'administrateur. Veuillez contacter le support.");
        }

        String stored = user.getMotDePasse();
        if (stored == null || stored.isEmpty())
            return null;
        if (!PasswordUtils.verifyPassword(motDePasse, stored))
            return null;
        return user;
    }
    ...
    /**
     * Reset password for a user by email (same email matching as login:
     * LOWER(email) = LOWER(?), trim in Java).
     */
    ...
    // Créer un objet User à partir du ResultSet en utilisant le polymorphisme
    // (instanceof)
    private User createUserFromResultSet(ResultSet rs) throws SQLException {
        ...
        if ("admin".equalsIgnoreCase(typeUtilisateur) || isAdmin(id)) {
            ...
            user = admin;
        } else if ("client".equalsIgnoreCase(typeUtilisateur) || isClient(id)) {
            ...
            client.setTypeUtilisateur("client");
            user = client;
        } else if ("guide".equalsIgnoreCase(typeUtilisateur) || isGuide(id)) {
            Guide guide = new Guide(id, nom, prenom, email, motDePasse, pays, imageurl);
            guide.setId_guide(id);
            guide.setTypeUtilisateur("guide");
            user = guide;
        } else if ("employee".equalsIgnoreCase(typeUtilisateur) || isModerateur(id)) {
            ...
            moderateur.setTypeUtilisateur("employee");
            user = moderateur;
        } else {
            user = new User(id, nom, prenom, email, motDePasse, pays, imageurl, typeUtilisateur);
        }
