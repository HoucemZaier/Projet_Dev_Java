...
public class PdfExportService {
    ...
    /**
     * Exporte un rapport statistique des hôtels et chambres.
     */
    public static void exporterRapportStatistiques(String path, java.util.List<com.PlaNova.models.Hotel> hotels,
            java.util.List<com.PlaNova.models.Chambre> chambres) {
        try {
            Document document = new Document();
            ...
            Font titleFont = FontFactory.getFont(FontFactory.HELVETICA_BOLD, 22);
            Paragraph titlePara = new Paragraph("Rapport Statistiques", titleFont);
            titlePara.setSpacingAfter(20);
            document.add(titlePara);

            Font bodyFont = FontFactory.getFont(FontFactory.HELVETICA, 12);

            document.add(new Paragraph("Nombre Total d'Hôtels: " + (hotels != null ? hotels.size() : 0), bodyFont));
            document.add(
                    new Paragraph("Nombre Total de Chambres: " + (chambres != null ? chambres.size() : 0), bodyFont));

            document.close();
        } catch (Exception e) {
            e.printStackTrace();
            System.err.println("Erreur lors de l'exportation du PDF.");
        }
    }
}
