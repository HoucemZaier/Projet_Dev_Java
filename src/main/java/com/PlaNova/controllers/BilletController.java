...
public class BilletController implements Initializable {
    ...
    @FXML
    private TextField idvField;
    @FXML
    private TextField numPlaceField;
    @FXML
    private TextField idDestinationField;
    @FXML
    private TextField idTransportPubField;
    @FXML
    private TextField idTransportPrivField;
    @FXML
    ...
    @FXML
    private Label idvError;
    @FXML
    private Label numPlaceError;
    @FXML
    private Label idDestinationError;
    @FXML
    private Label idTransportPubError;
    @FXML
    private Label idTransportPrivError;
    ...
    private void setupTable() {
        ...
        billetTable.getColumns().addAll(checkCol, numCol, idCol, dbCol, idvCol, placeCol, destCol, pubCol, privCol,
                actionsCol);
        billetTable.getStyleClass().addAll(Styles.BORDERED, Styles.STRIPED);
        billetTable.setColumnResizePolicy(TableView.CONSTRAINED_RESIZE_POLICY);
    ...
    private boolean validateForm() {
        ...
        if (!validateNumPlace())
            valid = false;
        if (!validateIdDestination())
            valid = false;
        if (!validateIdTransportPub())
            valid = false;
        if (!validateIdTransportPriv())
            valid = false;
    ...
    private void clearErrors() {
        for (Label err : new Label[] { dbError, idvError, numPlaceError, idDestinationError, idTransportPubError,
                idTransportPrivError }) {
            clearFieldError(err);
        }
        dbField.getStyleClass().remove("input-error");
        idvField.getStyleClass().remove("input-error");
