...
public class listHotelController {
    ...
    private HBox createEmptyRow() {
        HBox row = new HBox();
        row.setStyle(
                "-fx-background-color: white; -fx-border-color: #e2e8f0; -fx-border-width: 0 1.5 1.5 1.5; -fx-padding: 20; -fx-alignment: CENTER;");
     ...
     * nomLabel.
     * setStyle("-fx-text-fill: #334155; -fx-font-weight: bold; -fx-font-size: 13;"
     * );
     ...
     * Label imageLabel = new Label(hotel.getImage() != null &&
     * !hotel.getImage().isEmpty() ? "📷" : "Aucune");
     ...
     * row.getChildren().addAll(idCell, nomCell, villeCell, etoilesCell, imageCell,
     * actionsCell);
     * 
     * // Effet de survol
    ...
    private void updateStats() {
        if (statsLabel == null || allHotels == null || filteredHotels == null) {
            if (statsLabel != null)
    ...
    private void updatePagination() {
        ...
        if (resultatsLabel != null) {
            int start = filteredHotels.isEmpty() ? 0 : (currentPage - 1) * ITEMS_PER_PAGE + 1;
            int end = Math.min(currentPage * ITEMS_PER_PAGE, filteredHotels.size());
            resultatsLabel.setText(String.format("Affichage %d-%d de %d résultats", start, end, filteredHotels.size()));
        }

        if (actifsLabel != null) {
            actifsLabel.setText(String.valueOf(filteredHotels.size()));
        }
    }
    ...
    private void handleAjouterHotel() {
        try {
            FXMLLoader loader = new FXMLLoader(getClass().getResource("/ui/hotel/ajouterhotel.fxml"));
    ...
    private void handleModifierHotel(Hotel hotel) {
        try {
            FXMLLoader loader = new FXMLLoader(getClass().getResource("/ui/hotel/modifierhotel.fxml"));
            Parent root = loader.load();
        ...
        alert.showAndWait().ifPresent(response -> {
            if (response == ButtonType.OK) {
                try {
                    ...
                    showAlert("Succès", "Hôtel \"" + hotel.getNomHotel() + "\" supprimé avec succès!",
