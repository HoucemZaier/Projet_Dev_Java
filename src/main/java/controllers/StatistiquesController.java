...
public class StatistiquesController {
    ...
    // ===== KPI Labels =====
    @FXML private Label totalHotelsLabel;
    ...
    @FXML private Label revenusEstimesLabel;

    // ===== Charts =====
    @FXML private BarChart<String, Number>  chambresParHotelChart;
    @FXML private PieChart statutChart;
    @FXML private BarChart<String, Number>  prixParHotelChart;
    @FXML private BarChart<String, Number>  typeChambresChart;
    ...
    @FXML private Label occupationPctLabel;
    ...
    private void updateChambresParHotelChart() {
        if (chambresParHotelChart == null) return;
        ...
        Platform.runLater(() -> {
            series.getData().forEach(data -> {
                if (data.getNode() != null) {
                    data.getNode().setStyle("-fx-bar-fill: #2A3B4C; -fx-background-radius: 4 4 0 0;");
                }
            });
        });
    }
    ...
    private void updateStatutPieChart() {
        ...
        byStatut.forEach((statut, count) ->
                statutChart.getData().add(new PieChart.Data(statut + " (" + count + ")", count)));

        // Couleurs custom pour le pie chart
        Platform.runLater(() -> {
            String[] colors = {"#10b981", "#ef4444", "#f59e0b", "#3b82f6", "#8b5cf6"};
            int i = 0;
            for (PieChart.Data data : statutChart.getData()) {
                if (i < colors.length && data.getNode() != null) {
    ...
    private void updatePrixParHotelChart() {
        if (prixParHotelChart == null) return;
        ...
        Platform.runLater(() -> {
            String[][] seriesColors = {{"#10b981"}, {"#0DA2E7"}, {"#f59e0b"}};
            int si = 0;
            for (XYChart.Series<String, Number> s : prixParHotelChart.getData()) {
                final String color = seriesColors[Math.min(si, 2)][0];
                s.getData().forEach(d -> {
                    if (d.getNode() != null)
    ...
    private void updateTypeChambresChart() {
        if (typeChambresChart == null) return;
        ...
        String[] colors = {"#0DA2E7", "#8b5cf6", "#10b981", "#f59e0b", "#ef4444"};
        Platform.runLater(() -> {
            int i = 0;
    ...
    private void updateTableDetail() {
        ...
        for (Hotel hotel : hotels) {
            List<Chambre> hChambres = byHotel.getOrDefault(hotel.getIdHotel(), List.of());
            long disponibles = hChambres.stream().filter(c -> "Disponible".equalsIgnoreCase(c.getStatutChambre())).count();
    ...
    private HBox createTableRowHeader() {
        ...
        String[] headers = {"Hôtel", "Étoiles", "Chambres", "Disponibles", "Occupées", "Taux", "Prix Moy."};
        double[] widths = {220, 80, 80, 90, 80, 100, 100};
