...
public class DestinationController implements Initializable {
    ...
    @FXML private DatePicker arrivalPicker;
    @FXML private ImageView imagePreview;
    @FXML private Button saveBtn;

    @FXML private Label nameError;
    @FXML private Label countryError;
    @FXML private Label departureError;
    @FXML private Label arrivalError;
    ...
    private class DatePickerCell extends TableCell<Destination, LocalDate> {
        private final DatePicker picker = new DatePicker();
        private final boolean isDeparture;

        DatePickerCell(boolean isDeparture) {
            this.isDeparture = isDeparture;
            picker.setMaxWidth(Double.MAX_VALUE);
            picker.setEditable(true);
            picker.setOnAction(e -> {
                LocalDate newVal = picker.getValue();
                if (newVal == null) return;
                Destination d = getTableView().getItems().get(getIndex());
                if (isDeparture) {
                    if (d.getDateArrivee() != null && !newVal.isBefore(d.getDateArrivee())) {
                        showNotification("Departure must be before arrival", false);
                        cancelEdit();
                        return;
                    }
                    d.setDateDep(newVal);
                } else {
                    if (d.getDateDep() != null && !newVal.isAfter(d.getDateDep())) {
                        showNotification("Arrival must be after departure", false);
                        cancelEdit();
                        return;
                    }
                    d.setDateArrivee(newVal);
                }
                commitEdit(newVal);
                saveInline(d);
            });
        ...
        public void cancelEdit() {
            super.cancelEdit();
            setText(getItem() != null ? getItem().format(DATE_FMT) : "");
            setGraphic(null);
        }
    ...
    private void setupTable() {
        ...
        checkCol.setCellFactory(col -> new TableCell<>() {
            private final CheckBox cb = new CheckBox();
            {
                cb.setOnAction(e -> {
                    Destination d = getTableView().getItems().get(getIndex());
                    if (cb.isSelected()) checkedItems.add(d);
                    else checkedItems.remove(d);
                    updateSelectionUI();
                });
            }
            @Override
        ...
        imgCol.setCellFactory(col -> new TableCell<>() {
            ...
            { iv.setFitHeight(40); iv.setFitWidth(55); iv.setPreserveRatio(true); iv.setSmooth(true); }
            @Override
            protected void updateItem(String url, boolean empty) {
                ...
                if (empty || url == null || url.isEmpty()) {
                    ...
                    try {
                        ...
                        setGraphic(iv); setText(null); setAlignment(Pos.CENTER);
                    } catch (Exception e) { setGraphic(null); setText("-"); setAlignment(Pos.CENTER); }
                }
            }
        });
        ...
        TableColumn<Destination, LocalDate> depCol = new TableColumn<>("Departure");
        depCol.setCellValueFactory(new PropertyValueFactory<>("dateDep"));
        depCol.setPrefWidth(140);
        depCol.setCellFactory(col -> new DatePickerCell(true));

        TableColumn<Destination, LocalDate> arrCol = new TableColumn<>("Arrival");
        arrCol.setCellValueFactory(new PropertyValueFactory<>("dateArrivee"));
        arrCol.setPrefWidth(140);
        arrCol.setCellFactory(col -> new DatePickerCell(false));

        // 9. Duration column (computed)
        TableColumn<Destination, String> durCol = new TableColumn<>("Duration");
        durCol.setCellValueFactory(cellData -> {
            long days = ChronoUnit.DAYS.between(
                cellData.getValue().getDateDep(),
                cellData.getValue().getDateArrivee());
        ...
        destinationTable.getColumns().addAll(
            ...
            depCol, arrCol, durCol, actionsCol
        );
        ...
        destinationTable.setRowFactory(tv -> {
            ...
            toggleItem.setOnAction(e -> {
                ...
                if (checkedItems.contains(d)) checkedItems.remove(d);
                else checkedItems.add(d);
                destinationTable.refresh();
                updateSelectionUI();
            });
            ...
            row.setOnMouseEntered(e -> {
                if (row.getItem() != null) {
                    ...
                    Tooltip tip = new Tooltip(
                        ...
                        "Country: " + d.getPays() + "\n" +
                        "Departure: " + d.getDateDep().format(DATE_FMT) + "\n" +
                        "Arrival: " + d.getDateArrivee().format(DATE_FMT) + "\n" +
                        "Duration: " + ChronoUnit.DAYS.between(d.getDateDep(), d.getDateArrivee()) + " days"
                    );
    ...
    void handleBatchDelete(ActionEvent event) {
        ...
        if (result.isPresent() && result.get() == ButtonType.OK) {
            ...
            for (Destination d : new ArrayList<>(checkedItems)) {
                try { service.delete(d); } catch (Exception ignored) {}
            }
            checkedItems.clear();
    ...
    void handleExportCsv(ActionEvent event) {
        ...
        if (file != null) {
            try (PrintWriter writer = new PrintWriter(file, "UTF-8")) {
                writer.println("ID,Name,Country,Departure,Arrival,Image");
    ...
    private void copyToClipboard(Destination d) {
        String text = String.join("\t",
            ...
            d.getNomDestination(),
            d.getPays(),
            d.getDateDep().format(DATE_FMT),
            d.getDateArrivee().format(DATE_FMT)
        );
    ...
    void handleUndo(ActionEvent event) {
        ...
        for (Destination d : items) {
            try { service.add(d); restored++; } catch (Exception ignored) {}
        }
        loadData();
    ...
    private void setupFormValidation() {
        nameField.textProperty().addListener((obs, o, n) -> {
            if (nameError.isVisible()) validateName();
        });
        countryField.textProperty().addListener((obs, o, n) -> {
            if (countryError.isVisible()) validateCountry();
        });
        departurePicker.valueProperty().addListener((obs, o, n) -> {
            if (departureError.isVisible()) validateDeparture();
            if (arrivalError.isVisible()) validateArrival();
        });
        arrivalPicker.valueProperty().addListener((obs, o, n) -> {
            if (arrivalError.isVisible()) validateArrival();
        });
    }
    ...
    private void updateStats() {
        ...
        long upcoming = filteredList.stream()
            .filter(d -> d.getDateDep().isAfter(LocalDate.now()))
            .count();
    ...
    private void showEditForm(Destination d) {
        ...
        departurePicker.setValue(d.getDateDep());
        arrivalPicker.setValue(d.getDateArrivee());
    ...
    void handleSave(ActionEvent event) {
        ...
        try {
            if (editingDestination == null) {
                ...
                editingDestination.setDateArrivee(arrivalPicker.getValue());
                editingDestination.setDateDep(departurePicker.getValue());
    ...
    private boolean validateForm() {
        ...
        if (!validateDeparture()) valid = false;
        if (!validateArrival()) valid = false;
    ...
    private boolean validateDeparture() {
        if (departurePicker.getValue() == null) {
            showFieldError(departureError, "Departure date is required");
            return false;
        }
        clearFieldError(departureError);
        return true;
    }

    private boolean validateArrival() {
        if (arrivalPicker.getValue() == null) {
            showFieldError(arrivalError, "Arrival date is required");
            return false;
        }
        if (departurePicker.getValue() != null
                && !arrivalPicker.getValue().isAfter(departurePicker.getValue())) {
            showFieldError(arrivalError, "Arrival date must be after departure");
            return false;
        }
        clearFieldError(arrivalError);
        return true;
    }
    ...
    private void clearErrors() {
        for (Label err : new Label[]{nameError, countryError, departureError, arrivalError}) {
            clearFieldError(err);
        }
        nameField.getStyleClass().remove("input-error");
        countryField.getStyleClass().remove("input-error");
