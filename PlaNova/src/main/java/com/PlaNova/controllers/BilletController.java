...
public class BilletController implements Initializable {
    ...
    private void setupKeyboardShortcuts() {
        billetTable.setOnKeyPressed(e -> {
            if (e.getCode() == KeyCode.DELETE) {
                if (!checkedItems.isEmpty()) handleBatchDelete(null);
                else {
                    Billet sel = billetTable.getSelectionModel().getSelectedItem();
                    if (sel != null) handleDelete(sel);
                }
                e.consume();
            } else if (e.isControlDown() && e.getCode() == KeyCode.D) {
                Billet sel = billetTable.getSelectionModel().getSelectedItem();
                if (sel != null) handleDuplicate(sel);
                e.consume();
                ...
                Billet sel = billetTable.getSelectionModel().getSelectedItem();
                if (sel != null) copyToClipboard(sel);
                e.consume();
            }
        });
    }
